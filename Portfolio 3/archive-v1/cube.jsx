...
function FrontFace() {
  return (<>
    ...
    <h2 className="cube__title">Leo Stider<br/>UX-design<br/><em style={{color:'var(--fg-muted)'}}>Stockholm</em></h2>
...
function ProjectsFace({ onNavigate }) {
  ...
  return (<>
    ...
    <div className="cube__sub">2023 — 2025</div>
  </>);
}
...
function AboutFace({ onNavigate }) {
  return (<>
    ...
    <p className="cube__big">A UX-designer<br/>who listens<br/>first.</p>
...
function ContactFace() {
  return (<>
    <div className="cube__corner" style={{color: 'var(--paper)', opacity: 0.7}}>Contact · 04 / 06</div>
...
function ResumeFace({ onNavigate }) {
  return (<>
    ...
    <p className="cube__big">Read:<br/><span style={{ background: 'var(--accent)', padding: '0 10px', marginLeft: -2 }}>45 sec</span></p>
...
function StatusFace() {
  return (<>
    ...
    <div>
      <p className="cube__big" style={{ fontSize: 40 }}>Currently<br/>designing at<br/>inamo.</p>
      <p style={{ marginTop: 16, fontSize: 14, lineHeight: 1.4 }}>Open to senior UX roles<br/>from May 2026.</p>
    </div>
...
const FACE_VARIANT = {
  ...
  left: "paper", top: "ink", bottom: "paper",
};

function Cube({ size = CUBE_SIZE, onNavigate }) {
  ...
  const stateRef = useR1({ rotX: -18, rotY: -28, startX: 0, startY: 0, baseX: 0, baseY: 0, dragging: false });
  ...
  const snapToNearest = useCb1(() => {
    const s = stateRef.current;
    let best = FACES[0], bestZ = -Infinity;
    for (const f of FACES) {
      const fxR = f.rx * Math.PI / 180, fyR = f.ry * Math.PI / 180;
      let nx = 0, ny = 0, nz = 1;
      const ny1 = ny * Math.cos(fxR) - nz * Math.sin(fxR);
      const nz1 = ny * Math.sin(fxR) + nz * Math.cos(fxR);
      ny = ny1; nz = nz1;
      const nx1 = nx * Math.cos(fyR) + nz * Math.sin(fyR);
      const nz2 = -nx * Math.sin(fyR) + nz * Math.cos(fyR);
      nx = nx1; nz = nz2;
      const rxR = s.rotX * Math.PI / 180, ryR = s.rotY * Math.PI / 180;
      const ny2 = ny * Math.cos(rxR) - nz * Math.sin(rxR);
      const nz3 = ny * Math.sin(rxR) + nz * Math.cos(rxR);
      ny = ny2; nz = nz3;
      const nx2 = nx * Math.cos(ryR) + nz * Math.sin(ryR);
      const nz4 = -nx * Math.sin(ryR) + nz * Math.cos(ryR);
      nx = nx2; nz = nz4;
      if (nz > bestZ) { bestZ = nz; best = f; }
    }
    const wrap = (cur, target) => {
      const diff = ((target - cur) % 360 + 540) % 360 - 180;
      return cur + diff;
    };
    s.rotX = wrap(s.rotX, -best.rx);
    s.rotY = wrap(s.rotY, -best.ry);
    setRotX(s.rotX); setRotY(s.rotY);
  }, []);
